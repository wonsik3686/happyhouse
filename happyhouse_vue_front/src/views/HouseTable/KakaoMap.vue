...
export default {
  ...
  mounted() {
    if (window.kakao && window.kakao.maps) {
      ...
      script.src =
        ...
        "1f2b05b74809adf220a85901b33cd3da" +
  ...
  methods: {
    ...mapActions("houseStore", ["setSelectDeal", "setSelectRent"]),
    initMap() {
              ...
              if (status === kakao.maps.services.Status.OK) {
                ...
                if (self.pay == "deal") {
                  inContent +=
                    ...
                    vo.dealAmount +
                    `</span></div>`;
                } else if (vo.rentMoney == "0") {
                  inContent +=
                    `</span><span class="price"> 보증금` +
                    vo.deposit +
                    `</span></div>`;
                } else {
                  inContent +=
      ...
      geocoder.addressSearch(
        this.sidoName + " " + this.gugunName + " " + this.dongName,
        function (result, status) {
          if (status === kakao.maps.services.Status.OK) {
            var coords = new kakao.maps.LatLng(result[0].y, result[0].x);
    ...
    addMarker(position, order) {
      ...
      var imageSrc =
        ...
        marker_env = new kakao.maps.Marker({
          position: position, // 마커의 위치
          image: markerImage,
        });
      marker_env.setMap(this.map); // 지도 위에 마커를 표출합니다
      this.markers_env.push(marker_env); // 배열에 생성된 마커를 추가합니다
      return marker_env;
    },
    // 지도 위에 표시되고 있는 마커를 모두 제거합니다
    removeMarker() {
      for (var i = 0; i < this.markers_env.length; i++) {
        this.markers_env[i].setMap(null);
      }
      this.markers_env = [];
    },
  },
